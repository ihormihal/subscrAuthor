subscrAuthor.grid.Items = function(config) {
	...
	Ext.applyIf(config,{
		...
		,tbar: [{
			text: _('subscrauthor_item_create')
...
Ext.extend(subscrAuthor.grid.Items,MODx.grid.Grid,{
	...
	,getMenu: function() {
        ...
        if (cs.split(',').length > 1) {
            m.push({
    			text: _('subscrauthor_items_remove')
    			,handler: this.removeSelected
    		});
        } else {
    		m.push({
    			text: _('subscrauthor_item_update')
    			,handler: this.updateItem
    		});
    		m.push('-');
    		m.push({
    			text: _('subscrauthor_item_remove')
	...
	,createItem: function(btn,e) {
		if (!this.windows.createItem) {
			this.windows.createItem = MODx.load({
				xtype: 'subscrauthor-window-item-create'
				,listeners: {
				...
				success: {fn:function(r) {
					if (!this.windows.updateItem) {
						this.windows.updateItem = MODx.load({
							xtype: 'subscrauthor-window-item-update'
							,record: r
							,listeners: {
								'success': {fn:function() { this.refresh(); },scope:this}
							}
	...
	,removeItem: function(btn,e) {
		...
		MODx.msg.confirm({
			title: _('subscrauthor_item_remove')
			,text: _('subscrauthor_item_remove_confirm')
			,url: this.config.url
			,params: {
    ...
    ,removeSelected: function(act,btn,e) {
        ...
        MODx.msg.confirm({
			title: _('subscrauthor_items_remove')
			,text: _('subscrauthor_items_remove_confirm')
			,url: this.config.url
			,params: {
...
Ext.reg('subscrauthor-grid-items',subscrAuthor.grid.Items);
...
subscrAuthor.window.CreateItem = function(config) {
	...
	Ext.applyIf(config,{
		title: _('subscrauthor_item_create')
		,id: this.ident
		,height: 200
		,width: 475
		,url: subscrAuthor.config.connector_url
...
Ext.reg('subscrauthor-window-item-create',subscrAuthor.window.CreateItem);
...
subscrAuthor.window.UpdateItem = function(config) {
	...
	Ext.applyIf(config,{
		title: _('subscrauthor_item_update')
		,id: this.ident
		,height: 200
		,width: 475
		,url: subscrAuthor.config.connector_url
...
Ext.reg('subscrauthor-window-item-update',subscrAuthor.window.UpdateItem);